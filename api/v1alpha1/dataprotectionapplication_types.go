...
func (in *LoadConcurrency) DeepCopyInto(out *LoadConcurrency) {
	*out = *in
	...
	// Deep copy the embedded LoadConcurrency fields
	if in.PerNodeConfig != nil {
		out.PerNodeConfig = make([]nodeagent.RuledConfigs, len(in.PerNodeConfig))
		for i, config := range in.PerNodeConfig {
			out.PerNodeConfig[i] = nodeagent.RuledConfigs{
				Number: config.Number,
			}
			
			// Deep copy the NodeSelector
			if config.NodeSelector.MatchLabels != nil {
				out.PerNodeConfig[i].NodeSelector.MatchLabels = make(map[string]string)
				for k, v := range config.NodeSelector.MatchLabels {
					out.PerNodeConfig[i].NodeSelector.MatchLabels[k] = v
				}
			}
			
			// Deep copy MatchExpressions if they exist
			if len(config.NodeSelector.MatchExpressions) > 0 {
				out.PerNodeConfig[i].NodeSelector.MatchExpressions = make([]metav1.LabelSelectorRequirement, len(config.NodeSelector.MatchExpressions))
				for j, expr := range config.NodeSelector.MatchExpressions {
					out.PerNodeConfig[i].NodeSelector.MatchExpressions[j] = metav1.LabelSelectorRequirement{
						Key:      expr.Key,
						Operator: expr.Operator,
					}
					if expr.Values != nil {
						out.PerNodeConfig[i].NodeSelector.MatchExpressions[j].Values = make([]string, len(expr.Values))
						copy(out.PerNodeConfig[i].NodeSelector.MatchExpressions[j].Values, expr.Values)
					}
				}
			}
		}
...
// Clones velero pkg/nodeagent/node_agent.go type Configs struct
